...
def picky_piggy(score):
    ...
    if score == 0:
        ...
        dice = make_test_dice(1, 4, 2, 8, 5, 7)
        while score:
            n = dice()
            score -= 1
        return n 
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # BEGIN PROBLEM 3
    if num_rolls == 0:
        return picky_piggy(score = opponent_score)
    else:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if not who:
            score0 += take_turn(strategy0, score1, dice=six_sided, goal=GOAL_SCORE)
            hog_pile(score0, score1)
            who = next_player(who)
        else:
            score1 += take_turn(strategy1, score0, dice=six_sided, goal=GOAL_SCORE)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def compare(score0, score1):
        ...
        if increase > running_high:
            print(f"{increase} point(s)! That's a record gain for Player {who}!")
            highest_increase = increase
        else:
            highest_increase = running_high
        return announce_highest(who=1, last_score=score1, running_high=highest_increase)
    return compare
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    def averaged_dice(*args):
        ...
        while i < trials_count:
            results, i = results + original_function(*args), i + 1
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while n <= 10:
        ...
        n_results = average_dice(n, dice)
        if n_results > max_score:
            max_score, n_max = n_results, n
        n = n + 1
    return n_max
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    new_score = score + picky_piggy(opponent_score) 
    if hog_pile(new_score, opponent_score) > new_score:
        return 0
    else:
